...
func (suite *KeeperTestSuite) SetupIBCTest() {
	...
	// Mint coins on the osmosis side which we'll use to unlock our dhives
	...
	// Mint coins on the cosmos side which we'll use to unlock our dhives
...
// Send back coins (from path endpoint B to A). In case of IBC coins need to provide ibcCoinMetadata (<port>/<channel>/<denom>, e.g.: "transfer/channel-0/dhives") as input parameter.
