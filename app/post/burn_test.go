...
func (s *PostTestSuite) TestPostHandle() {
	testCases := []struct {
		...
		},
		{
			...
			tx: func() sdk.Tx {
				feeAmount := sdk.Coins{sdk.Coin{Amount: sdkmath.NewInt(10), Denom: "dhives"}}
				amount := sdk.Coins{sdk.Coin{Amount: sdkmath.NewInt(20), Denom: "dhives"}}
				s.MintCoinsForFeeCollector(amount)
			...
			expPass: true,
			postChecks: func() {
				expected := sdk.Coins{sdk.Coin{Amount: sdkmath.NewInt(10), Denom: "dhives"}}
				balance := s.GetFeeCollectorBalance()
				s.Require().Equal(expected, balance)
			},
		},
		{
			...
			tx: func() sdk.Tx {
				feeAmount := sdk.Coins{
					...
					sdk.Coin{Amount: sdkmath.NewInt(10), Denom: "dhives"},
				}
				amount := feeAmount
				s.MintCoinsForFeeCollector(amount)
		...
		},
		{
			...
			tx: func() sdk.Tx {
				...
				s.Require().True(ok)
				feeAmount := sdk.Coins{sdk.Coin{Amount: amt, Denom: "dhives"}}
				amount := sdk.Coins{sdk.Coin{Amount: amt, Denom: "dhives"}}
				s.MintCoinsForFeeCollector(amount)
